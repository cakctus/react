...
import UpdateForm from "../Forms/UpdateAPI"
...
function PostId() {
  ...
  React.useEffect(() => {
    axios.get(`http://127.0.0.1:8000/api/items/${params.id}/`).then(
      (response) => {
  ...
  function addToCard() {
    axios.get(`http://127.0.0.1:8000/add/${params.id}`).then(() => {
      alert("Item added to card")
    })
  }

  function removeToCard() {
    axios.get(`http://127.0.0.1:8000/remove/${params.id}`).then(() => {
      alert("Item  removed to card!")
    })
  }

  function deletePost() {
    axios.delete(`http://127.0.0.1:8000/api/items/${params.id}`).then(() => {
      alert("Post deleted!")
    })
  }
