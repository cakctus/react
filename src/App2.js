...
function App() {
  ...
  return (
    <div className='App'>
      ...
      <Select options={[{value: 'title', name: 'title'}, {value: 'date', name: 'date'}]} 
      ...
      {loading === true ? <Loader/> : <PostList post={post} postTogle={postTogle} remove={removePost}/>}
      {loading2 && <Loader2/>}
      


      {pageArray.map(p => 
      <button onClick={() => changePage(p)} key={p} className={page === p ? 'current_page': 'just_page'}>{p}</button> 
      )}
